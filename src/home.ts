  ...
  if (token) {
    fetch(`${SERVER_URL}/verify`, {
      ...
      .then(async ({ valid }) => {
        ...
        let tanks: Tank[] = await getTanks(token, userData.tanks);
...
function hideTankList(e: MouseEvent) {
  const tankList: HTMLUListElement = document.querySelector(".tank-list");
...
function renderTank(tank: Tank, i: number) {
  // add click handler
...
function displayInitalData(tanks: Tank[]) {
...
function displaySelectedTank(tank: Tank) {}

function displayTanksInit(tanks:Tank[]) {
  return tanks.map((tank: Tank) => createTankLarge(tank));
}

function createTankLarge(tank: Tank) {
...
function getTanks(token: string, tankIds: string[]): Tank[] {
